...
- Runtime dirs `data/ review/ publish/ renders/` are gitignored; create them via
  `config.ensure_dirs()`.
- Generation runs under the **Codex Pro/Max subscription** by default
  (`model.backend: claude_cli` shells out to the `Codex` CLI — no API key).
  Host needs Codex installed + `Codex login`. An `api` backend
