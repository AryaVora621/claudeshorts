...
def list_articles(limit: int = 120) -> list[dict[str, Any]]:
    return service_call(articles_service.list_articles, limit)


@router.post("")
def add_article(body: AddArticleRequest) -> dict[str, Any]:
    return service_call(
        articles_service.add_manual_article,
...
def pin(item_id: int) -> dict[str, Any]:
    return service_call(articles_service.pin_article, item_id)


@router.post("/{item_id}/unpin")
def unpin(item_id: int) -> dict[str, Any]:
    return service_call(articles_service.unpin_article, item_id)


@router.post("/{item_id}/generate", response_model=EnqueueResponse)
def generate(item_id: int) -> dict[str, Any]:
    return service_call(articles_service.generate_from_item, item_id)
