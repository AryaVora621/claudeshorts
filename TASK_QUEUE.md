...
- [IN_PROGRESS Claude/Fable5] goal.md rebuild implementation, chunks 1-8 via
  ...
  **Chunks 1-2 code-complete** (chunk 1: store layer fully Postgres, 31 tests;
  chunk 2: durable job queue + state machine + polling worker + dashboard
  rewire, verified live end-to-end, 55/55 tests). Only chunk 1's human-required
  Task 11 (real data migration) remains — see Open. Progress ledger:
  `.superpowers/sdd/progress.md`. Next: chunk 3 (service layer).
